...
def evaluate(data, knntable, k, p, l, f, t, v):
    ...
    approxknn = forestknn(data, ensembler(data, myforest, v), k, v)
    return recall(data, k, knntable, approxknn), discrepancyav(data, k, knntable, approxknn)


parser = argparse.ArgumentParser(description='Randomized PCA Forest')
...
parser.add_argument("-p", "--principalcomponents", help="Number of principal components to use.", default=1, type = int)
...
for i in range(args.threads):
    size = base_size + (1 if i < remainder else 0)  
    thread = threading.Thread(target=fitforest, args=(data, forests[i], size, args.principalcomponents, args.leafsize, args.verbos))
    threads.append(thread)
    thread.start()
for thread in threads:
    thread.join()
myforest = sum((forest[0] for forest in forests), [])
approxknn = forestknn(data, ensembler(data, myforest, args.verbos), args.k, args.verbos)
