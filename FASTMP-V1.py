...
npc = 5
weight = np.empty(npc)
print("Enter weights of each " + str(npc) + " principal components")
for i in range(npc):
    if i % 20 == 0:
        weight[i] = input(str(i+1) + "st:")
    elif i % 20 == 1:
        weight[i] = input(str(i+1) + "nd:")
    elif i % 20 == 2:
        weight[i] = input(str(i+1) + "rd:")
    else:
        weight[i] = input(str(i+1) + "th:")
data = np.loadtxt('4/full.csv', dtype=float, delimiter=',')
start = time.time()
rpca = PCA(n_components = npc, svd_solver = 'randomized')
rpca.fit(data)
datapca = rpca.transform(data)
projdata = np.empty([data.shape[0], 1])
for i in range(data.shape[0]):
    temp = 0
    for j in range(npc):
        temp = temp + weight[j] * datapca[i, j]
    projdata[i, 0] = temp
print(projdata)  
forest1 = []
forest2 = []
